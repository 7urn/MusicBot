...
function initializePlayer(client) {
    ...
    client.riffy.on("nodeConnect", node => {
        console.log(`Nodo "${node.name}" connesso.`);
    });

    client.riffy.on("nodeError", (node, error) => {
        console.error(`Il nodo "${node.name}" ha incontrato un errore: ${error.message}.`);
    });
    ...
    client.riffy.on("trackStart", async (player, track) => {
        ...
        try {
            const musicard = await Dynamic({
                ...
                nameColor: '#FF7A00',
                author: track.info.author || 'Artista sconosciuto',
                authorColor: '#696969',
            });
            ...
            console.error("Errore durante la creazione o l'invio della musicard:", error.message);
            const errorEmbed = new EmbedBuilder()
                .setColor('#FF0000')
                .setDescription("⚠️ **Impossibile caricare la track Card. Continuando la riproduzione...**");
    ...
    client.riffy.on("queueEnd", async (player) => {
        ...
        if (channel && currentTrackMessageId) {
            const queueEmbed = new EmbedBuilder()
                ...
                .setDescription('**Le canzoni in coda sono finite! Disconnessione del Bot!**');
    ...
    async function disableTrackMessage(client, player) {
        ...
        try {
            ...
            console.error("Impossibile disabilitare i componenti del messaggio:", error);
        }
    }
}
    ...
    collector.on('collect', async i => {
        ...
        if (!voiceChannel || voiceChannel.id !== playerChannel) {
            const vcEmbed = new EmbedBuilder()
                .setColor(config.embedColor)
                .setDescription('🔒 **Devi essere nello stesso canale vocale per usare i controlli!**');
...
async function handleInteraction(i, player, channel) {
    switch (i.customId) {
        ...
        case 'skipTrack':
            player.stop();
            await sendEmbed(channel, "⏭️ **Il Player riprodurrà la canzone successiva!**");
            break;
        case 'disableLoop':
        ...
        case 'stopTrack':
            player.stop();
            player.destroy();
            await sendEmbed(channel, '⏹️ **La riproduzione è stata interrotta e il bote è stato disconnesso!**');
            break;
        case 'pauseTrack':
            if (player.paused) {
        ...
        case 'resumeTrack':
            if (!player.paused) {
                ...
                player.pause(false);
                await sendEmbed(channel, '▶️ **La riproduzione è stata ripresa!**');
            }
            break;
        case 'volumeUp':
...
function adjustVolume(player, channel, amount) {
    ...
    if (newVolume === player.volume) {
        ...
        sendEmbed(channel, `🔊 **Il volume è cambiato a ${newVolume}%!**`);
    }
}
...
function disableLoop(player, channel) {
    player.setLoop("none");
    sendEmbed(channel, "❌ **Loop disattivato!**");
}

function showQueue(channel) {
    if (queueNames.length === 0) {
        sendEmbed(channel, "La coda è vuota.");
        return;
    }

    const nowPlaying = `🎵 **Ora in riproduzione:**\n${formatTrack(queueNames[0])}`;
    const queueChunks = [];
